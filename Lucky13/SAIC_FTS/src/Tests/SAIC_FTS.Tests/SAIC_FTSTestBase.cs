...
namespace SAIC_FTS.Tests
{
    public abstract class SAIC_FTSTestBase : AbpIntegratedTestBase<SAIC_FTSTestModule>
    {
        ...
        protected void UsingDbContext(Action<SAIC_FTSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SAIC_FTSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SAIC_FTSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SAIC_FTSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SAIC_FTSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SAIC_FTSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SAIC_FTSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SAIC_FTSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SAIC_FTSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SAIC_FTSDbContext, Task<T>> func)
        {
            T result;
